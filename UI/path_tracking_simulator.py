...
class Path_Tracking_Simulator(QDialog):
    ...
    def update_path_image(self):
        ...
        # 保存到源目录
        src_path = os.environ['HOME'] + '/path_tracking/src/robot_simulation/robot_gazebo/worlds/path_tracking_stage/meshes/stage.png'
        cv2.imwrite(src_path, frame)
        print(f"Path image saved to: {src_path}")

        # 保存到安装目录（如果存在）
        install_dir = Path(os.environ['HOME'] + '/path_tracking/install/robot_gazebo/share/robot_gazebo/worlds/path_tracking_stage/meshes')
        if install_dir.exists():
            install_path = str(install_dir / 'stage.png')
            cv2.imwrite(install_path, frame)
            print(f"Path image saved to: {install_path}")
        print("Path image updated! Please restart Gazebo to see the changes.")
